...
class Old_Magnet():
    ...
    def __init__(self, magnet_file, session_df, mouse, date, process=False, 
                 ...
                 on_off_threshold=15, independent=5000):
    ...
    def detect_press_ind(self, smooth=15, plot=True, search_offset=50,
        ...
        for ind, press, threshold, curr_baseline in zip(range(len(LPs)), 
            ...
            if num_detected == 1:
                ...
                elif abs(search_end_ind - up_crossing[-1]) < 250:
            ...
            if plot:
                ...
                # Press indices
                plt.text(t[final_down_idx], m[final_down_idx], f'{ind} D',
                             fontsize=8)
...
def day_by_day(mouse_dict, length=500, vmin=0, vmax=1, traces=False, plot=False):
    ...
    for day_ind, vectors in mouse_dict.items():
        all_matrices[day_ind] = np.zeros((len(vectors), length))
        for press_ind, press in enumerate(vectors.values()):
            if len(press) < length:
                all_matrices[day_ind][press_ind,:len(press)] = press
            else:
                all_matrices[day_ind][press_ind,:length] = press[:length]
...
def by_press_no(all_matrices, master_df, mouse, length=1000,
                vmin=0, vmax=1, save=False):
    ...
    for n in range(5):
        plt.figure()
        plt.title(f'{mouse}, press no. #{n+1}')
    ...
    # Save
    if save:
        plt.savefig(f'press_no_{n}_{mouse}_{length}.png', dpi=500)
    return final_matrices
            
        
def in_vs_out_seq_corr(all_matrices, master_df, mouse, length=500,vmin_in=0, vmax_in=1,
                       vmin_out=0,vmax_out=1, save=False):
    mouse_df = master_df[master_df['Mouse'] == mouse]
    dates = sorted(list(mouse_dict.keys()))
    date_indices = [date_ind for date_ind in range(len(dates))]
...
if __name__ == '__main__':
    ...
    ### Process data ###
    # on_off_thresholds = {
    #     '20220209': 15, '20220210': 30, '20220211': 15,
    #     '20220213': 15, '20220214': 15, '20220215': 15,
    #     '20220216': 15, '20220218': 15, '20220221': 15,
    #     '20220222': 15, '20220224': 15
    #     }
    
    # independent_thresholds = {
    #     '20220209': 5_000, '20220210': 5_000, '20220211': 10_000,
    #     '20220213': 7_500, '20220214': 5_000, '20220215': 5_000,
    #     '20220216': 5_000, '20220218': 5_000, '20220221': 7_500,
    #     '20220222': 5_000, '20220224': 5_000
    #     }
    # session = Old_Magnet(magnet_file, date_df, mouse, date, process,
    #                       on_off_threshold=on_off_thresholds[date],
    #                       independent=independent_thresholds[date])
    # plot_what = ['first_last_press', 'scale_LPs']
    
    # session.optimize_thresholds()
    # session.detect_press_ind(plot=False, search_offset=10, hill_start=10,
    #                           hill_end=50)
    # session.plot(['presses', 'scale_LPs'])
    
    # final_LP_vectors = load_final_LP_vectors(session.magnet, 
    #                                           session.final_press_ind) 
    dates = ['20220209','20220210', '20220211',
    '20220213','20220214','20220215',
    '20220216','20220218','20220221', '20220224']
    import pickle
    with open('magnet_presses_final_4411.pkl', 'rb') as handle:
        mouse_dict = pickle.load(handle)
        mouse_dict.pop('20220222')
        length = 1000
        all_matrices, final_corr_matrix = day_by_day(mouse_dict, length=length, 
                                                      vmin=0, vmax=1, traces=False)
        # final_matrices = by_press_no(all_matrices, master_df, mouse, 
        #                                          length=length, vmin=0, vmax=1,
        #                                          save=False)
        in_seq_matrices, out_seq_matrices = in_vs_out_seq_corr(all_matrices,
                                                               master_df, mouse,
                                                               length=length,
                                                               vmin_in=0.2, vmax_in=0.5,
                                                               vmin_out=0.2, vmax_out=0.5,
                                                               save=False)
